	...
	const Symbol = (symbol) => {
		const content = Object.entries(site.content).reduce((accumulator, [locale, value]) => {
			...
			symbol.fields.forEach((field) => {
				accumulator[locale][field.key] = get_empty_value(field)
			})
			return accumulator
		}, {})
	...
	const Page = (page) => {
		const content = Object.entries(site.content).reduce((accumulator, [locale, value]) => {
			...
			page.fields.forEach((field) => {
				accumulator[locale][field.key] = value?.[page.id]?.[field.key] || get_empty_value(field)
			})
			return accumulator
		}, {})
...
export function validate_symbol(symbol) {
	if (!symbol.content || _.isEmpty(symbol.content)) {
		...
		const content = Object.entries(symbol.fields).reduce((accumulator, [_, field]) => {
			accumulator[field.key] = get_empty_value(field)
			return accumulator
		}, {})
